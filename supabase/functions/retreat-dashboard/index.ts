...
Deno.serve(async (req) => {
  ...
  try {
    const authError = validateRetreatDashboardAuth(req);
    if (authError) {
    ...
    const { data: bookingData, error: bookingError } = await admin
      .from('bookings')
      .select(
        'id, sanity_event_id, event_title, event_type, event_start, stripe_customer_name, stripe_customer_email, stripe_customer_whatsapp, amount_total, currency, payment_status, booking_status, created_at'
      )
      .ilike('event_type', '%retreat%')
    ...
    for (const booking of paidBookings) {
      const key = booking.sanity_event_id || booking.event_title || 'unknown-retreat';
      const title = booking.event_title || 'Untitled Retreat';
    ...
    const topEvents = [...eventTotals.values()].sort((a, b) => b.revenueCents - a.revenueCents);
    ...
    return json(req, {
      ...
      topEvents,
      recentBookings: bookings.map((row) => ({
        id: row.id,
...
function validateRetreatDashboardAuth(req: Request): string | null {
  const expectedUser = Deno.env.get('RETREAT_DASHBOARD_USERNAME') || Deno.env.get('DASHBOARD_USERNAME') || '';
  const expectedPass = Deno.env.get('RETREAT_DASHBOARD_PASSWORD') || Deno.env.get('DASHBOARD_PASSWORD') || '';
  if (!expectedUser || !expectedPass) {
    return 'Retreat dashboard credentials are not configured.';
  }
