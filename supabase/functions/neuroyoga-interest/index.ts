...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const scheduleNote = (payload.scheduleNote || 'Dates TBA based on group formation').trim();
    const source = (payload.source || 'neuroyoga-program-page').trim();
    ...
    const toEmail = (Deno.env.get('NEUROYOGA_INTEREST_TO') || 'info@awellyoga.com').trim();
    ...
    const subject = `New NeuroYoga Program Application (${startWindow})`;
    let text = [
      'A new NeuroYoga program application was submitted.',
      '',
      `Name: ${name}`,
      `Email: ${email}`,
    ...
    let html = `
      <div style="font-family:Arial,sans-serif;line-height:1.6;color:#1f2937;">
        ...
        <p style="margin:0 0 6px;"><strong>Schedule:</strong> ${escapeHtml(scheduleNote)}</p>
    ...
    return json(req, { ok: true, message: 'Application submitted.' });
  } catch (error) {
